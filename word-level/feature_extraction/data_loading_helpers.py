...
def extract_word_level_data(data_container, word_objects, eeg_float_resolution = np.float16):
    ...
    if "rawEEG" in available_objects:
        ...
        try:
            ...
            inSacc_amp = word_objects["inSacc_amp"]
        except KeyError:
            inSacc_velocity = []
            inSacc_duration = []
            outSacc_velocity = []
        ...
        for raw_eegs_obj, ets_obj, word_obj, ffd, gd, gpt, trt, nFix, fixPos, insacc_v, insacc_d, outsacc_v, outsacc_d, insacc_a, outsacc_a, trt_t1, trt_t2, trt_a1, trt_a2, trt_b1, trt_b2, trt_g1, trt_g2, ffd_t1, ffd_t2, ffd_a1, ffd_a2, ffd_b1, ffd_b2, ffd_g1, ffd_g2 in zipped_data:
            ...
            if is_real_word(word_string):
                ...
                data_dict["inSacc_velocity_mean"] = np.mean(data_container[insacc_v[0]])
                data_dict["inSacc_duration_mean"] = np.mean(data_container[insacc_d[0]])
                data_dict["outSacc_velocity_mean"] = np.mean(data_container[outsacc_v[0]])
                data_dict["outSacc_duration_mean"] = np.mean(data_container[outsacc_d[0]])
                data_dict["inSacc_amp_mean"] = np.mean(data_container[insacc_a[0]])
                data_dict["outSacc_amp_mean"] = np.mean(data_container[outsacc_a[0]])
                data_dict["inSacc_velocity_max"] = np.max(data_container[insacc_v[0]])
        ...
        for word_obj in contentData:
            ...
            if is_real_word(word_string):
                ...
                data_dict["TRT"] = None
                data_dict["nFix"] = None
                data_dict["inSacc_velocity_mean"] = None
                data_dict["inSacc_duration_mean"] = None
                data_dict["outSacc_velocity_mean"] = None
                data_dict["outSacc_duration_mean"] = None
                data_dict["inSacc_amp_mean"] = None
                data_dict["outSacc_amp_mean"] = None
                data_dict["inSacc_velocity_max"] = None
                data_dict["inSacc_duration_max"] = None
                data_dict["outSacc_velocity_max"] = None
                data_dict["outSacc_duration_max"] = None
                data_dict["inSacc_amp_max"] = None
                data_dict["outSacc_amp_max"] = None
                data_dict["ALPHA_EEG"] = []
                data_dict["BETA_EEG"] = []
                data_dict["GAMMA_EEG"] = []
                data_dict["THETA_EEG"] = []
