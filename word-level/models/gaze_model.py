...
def lstm_classifier_cross(labels_train, gaze_train, labels_test, gaze_test, param_dict, random_seed_value):
    ...
    sents_gaze_train = list(gaze_train.keys())

    if sents_y_train[0] != sents_gaze_train[0]:
        sys.exit("STOP! Order of sentences in labels and features dicts not the same!")
    ...
    # prepare gaze data
    gaze_X_train, max_length_cogni_train = ml_helpers.prepare_cogni_seqs(gaze_train)
    gaze_X_test, max_length_cogni_test = ml_helpers.prepare_cogni_seqs(gaze_test)
    #print(gaze_X.shape)
    gaze_X_train = ml_helpers.scale_feature_values(gaze_X_train)
    gaze_X_test = ml_helpers.scale_feature_values(gaze_X_test)
    #print(gaze_X.shape)
    X_data_train = ml_helpers.pad_cognitive_feature_seqs(gaze_X_train, max(max_length_cogni_train, max_length_cogni_test), "eye_tracking")
    X_data_test = ml_helpers.pad_cognitive_feature_seqs(gaze_X_test, max(max_length_cogni_train, max_length_cogni_test), "eye_tracking")
    ...
    # define model
    print("Preparing model...")
    input_gaze = Input(shape=(X_data_train.shape[1], X_data_train.shape[2]), name='gaze_input_tensor')
    gaze_model = Bidirectional(LSTM(lstm_dim, return_sequences=True))(input_gaze)
    for _ in list(range(lstm_layers-1)):
        gaze_model = Bidirectional(LSTM(lstm_dim, return_sequences=True))(gaze_model)
    gaze_model = Flatten()(gaze_model)
