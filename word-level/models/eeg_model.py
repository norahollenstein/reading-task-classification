...
def lstm_classifier(labels, eeg, param_dict, random_seed_value):
...
def lstm_classifier_cross(labels_train, eeg_train, labels_test, eeg_test, param_dict, random_seed_value):
    ...
    # check order of sentences in labels and features dicts
    sents_y_train = list(labels_train.keys())
    sents_eeg_train = list(eeg_train.keys())

    if sents_y_train[0] != sents_eeg_train[0]:
        sys.exit("STOP! Order of sentences in labels and features dicts not the same!")

    y_train = list(labels_train.values())
    # convert class labels to one hot vectors
    y_train = np_utils.to_categorical(y_train)
    y_test = list(labels_test.values())
    y_test = np_utils.to_categorical(y_test)

    # prepare gaze data
    eeg_X_train, max_length_cogni_train = ml_helpers.prepare_cogni_seqs(eeg_train)
    eeg_X_test, max_length_cogni_test = ml_helpers.prepare_cogni_seqs(eeg_test)
    # print(gaze_X.shape)
    eeg_X_train = ml_helpers.scale_feature_values(eeg_X_train)
    eeg_X_test = ml_helpers.scale_feature_values(eeg_X_test)
    # print(gaze_X.shape)
    X_data_train = ml_helpers.pad_cognitive_feature_seqs(eeg_X_train,
                                                         max(max_length_cogni_train, max_length_cogni_test),
                                                         "eeg")
    X_data_test = ml_helpers.pad_cognitive_feature_seqs(eeg_X_test, max(max_length_cogni_train, max_length_cogni_test),
                                                        "eeg")
    ...
    # define model
    print("Preparing model...")
    input_text = Input(shape=(X_data_train.shape[1], X_data_train.shape[2]), name='eeg_input_tensor')
    ...
    # train model
    history = model.fit(X_data_train, y_train, validation_split=config.validation_split, epochs=epochs, batch_size=batch_size, callbacks=[early_stop,model_save])
    print("Best epoch:",len(history.history['loss'])-config.patience)
    ...
    scores = model.evaluate(X_data_test, y_test, verbose=0)
